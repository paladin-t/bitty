...
\returns Returns 0 on success, -1 on failure.
*/
int hlineDraw(SDL_Renderer * renderer, Sint16 x1, Sint16 x2, Sint16 y)
{
	...
	do {
		...
		if (ocy != cy) {
			if (cy > 0) {
				ypcy = y + cy;
				ymcy = y - cy;
				result |= hlineDraw(renderer, xmcx, xpcx + dx, ypcy + dy);
				result |= hlineDraw(renderer, xmcx, xpcx + dx, ymcy);
			} else {
				result |= hlineDraw(renderer, xmcx, xpcx + dx, y);
			}
			ocy = cy;
		}
		if (ocx != cx) {
			if (cx != cy) {
				if (cx > 0) {
					ypcx = y + cx;
					ymcx = y - cx;
					result |= hlineDraw(renderer, xmcy, xpcy + dx, ymcx);
					result |= hlineDraw(renderer, xmcy, xpcy + dx, ypcx + dy);
				} else {
					result |= hlineDraw(renderer, xmcy, xpcy + dx, y);
				}
			}
			ocx = cx;
		}
...
\returns Returns 0 on success, -1 on failure.
*/
static int filledEllipseRGBA_(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rx, Sint16 ry, Uint8 r, Uint8 g, Uint8 b, Uint8 a);
int filledCircleColor(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rad, Uint32 color)
{
	Uint8 *c = (Uint8 *)&color;
	if (c[3] == 255)
		return filledEllipseRGBA_(renderer, x, y, rad, rad, c[0], c[1], c[2], c[3]); // FIXME.
	else
	...
	do {
		...
		if (ocy != cy) {
			if (cy > 0) {
				ypcy = y + cy;
				ymcy = y - cy;
				result |= hlineDraw(renderer, xmcx, xpcx, ypcy);
				result |= hlineDraw(renderer, xmcx, xpcx, ymcy);
			} else {
				result |= hlineDraw(renderer, xmcx, xpcx, y);
			}
			ocy = cy;
		}
		if (ocx != cx) {
			if (cx != cy) {
				if (cx > 0) {
					ypcx = y + cx;
					ymcx = y - cx;
					result |= hlineDraw(renderer, xmcy, xpcy, ymcx);
					result |= hlineDraw(renderer, xmcy, xpcy, ypcx);
				} else {
					result |= hlineDraw(renderer, xmcy, xpcy, y);
				}
			}
			ocx = cx;
		}
	...
	if (rx > ry) {
		...
		do {
			...
			if ((ok != k) && (oj != k)) {
				...
				xmh = x - h;
				if (k > 0) {
					result |= hlineDraw(renderer, xmh, xph, y + k);
					result |= hlineDraw(renderer, xmh, xph, y - k);
				} else {
					result |= hlineDraw(renderer, xmh, xph, y);
				}
				ok = k;
			}
			if ((oj != j) && (ok != j) && (k != j)) {
				...
				xpi = x + i;
				if (j > 0) {
					result |= hlineDraw(renderer, xmi, xpi, y + j);
					result |= hlineDraw(renderer, xmi, xpi, y - j);
				} else {
					result |= hlineDraw(renderer, xmi, xpi, y);
				}
				oj = j;
			}
		...
		do {
			...
			if ((oi != i) && (oh != i)) {
				...
				xpj = x + j;
				if (i > 0) {
					result |= hlineDraw(renderer, xmj, xpj, y + i);
					result |= hlineDraw(renderer, xmj, xpj, y - i);
				} else {
					result |= hlineDraw(renderer, xmj, xpj, y);
				}
				oi = i;
			}
			if ((oh != h) && (oi != h) && (i != h)) {
				...
				xpk = x + k;
				if (h > 0) {
					result |= hlineDraw(renderer, xmk, xpk, y + h);
					result |= hlineDraw(renderer, xmk, xpk, y - h);
				} else {
					result |= hlineDraw(renderer, xmk, xpk, y);
				}
				oh = h;
			}
...
int filledEllipseRGBA_(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rx, Sint16 ry, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	int result;
	int ix, iy;
	int h, i, j, k;
	...
	for (y = miny; (y <= maxy); y++) {
		...
		*/
		...
		for (i = 0; (i < ints); i += 2) {
			...
			result |= hlineDraw(renderer, xa, xb, y);
		}
	}
...
int texturedPolygonMT2(SDL_Renderer *renderer, const Sint16 * vx, const Sint16 * vy, int n,
	SDL_Texture* texture, int texture_dx, int texture_dy, int **polyInts, int *polyAllocated, int w, int h)
{
	int result;
	int i;
	int y, xa, xb;
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i + 1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTextured(renderer, xa, xb, y, textureAsTexture, w, h, texture_dx, texture_dy);
		}
	}
...
int texturedPolygon2(SDL_Renderer *renderer, const Sint16 * vx, const Sint16 * vy, int n, SDL_Texture*texture, int texture_dx, int texture_dy, int w, int h)
{
	/*
	* Draw
	*/
	return (texturedPolygonMT2(renderer, vx, vy, n, texture, texture_dx, texture_dy, NULL, NULL, w, h));
}
...
int primitivePurge(void) {
	if (gfxPrimitivesPolyIntsGlobal) {
		free(gfxPrimitivesPolyIntsGlobal);
		gfxPrimitivesPolyIntsGlobal = NULL;
	}
	int ret = gfxPrimitivesPolyAllocatedGlobal;
