...
extern void jo_gif_frame(jo_gif_t *gif, unsigned char *rgba, short delayCsec, bool localPalette, bool paletteFilled);
...
static void jo_gif_quantize(unsigned char *rgba, int rgbaSize, int sample, unsigned char *map, int numColors) {
	...
	for (int i = 0; i < numColors; i++) {
		map[i*3+0] = (unsigned char)(network[i][0] >>= 4);
		map[i*3+1] = (unsigned char)(network[i][1] >>= 4);
		map[i*3+2] = (unsigned char)(network[i][2] >>= 4);
	}
}
...
void jo_gif_frame(jo_gif_t *gif, unsigned char * rgba, short delayCsec, bool localPalette, bool paletteFilled) {
	...
	unsigned char *indexedPixels = (unsigned char *)malloc(size);
	{
		...
		for(int k = 0; k < size*4; k+=4) {
			...
			indexedPixels[k/4] = (unsigned char)best;
